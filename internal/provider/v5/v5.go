...
func blockAttrs(block *tfplugin5.Schema_Block) []provider.Attr {
	var attrs []provider.Attr
...
// version-neutral provider.NestedBlock.
func nestedBlocks(block *tfplugin5.Schema_Block) []provider.NestedBlock {
...
func (b *Backend) ReadDataSource(ctx context.Context, req provider.ReadDataSourceRequest) (provider.ReadDataSourceResult, error) {
	...
	resp, err := b.client.ReadDataSource(ctx, &tfplugin5.ReadDataSource_Request{
	...
	attrs, err := decodeState(raw.objType, resp.GetState())
	if err != nil {
