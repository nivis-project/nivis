...
func TestApplyOneCreate(t *testing.T) {
	...
	if _, _, err := d.applyOne(context.Background(), g, node, map[string]interface{}{}, nil); err != nil {
		t.Fatal(err)
	}
	if c.destroyCalls != 0 {
...
func TestApplyOneReplaceDestroysThenCreates(t *testing.T) {
	...
	attrs, _, err := d.applyOne(context.Background(), g, node, map[string]interface{}{}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if c.destroyCalls != 1 {
...
func TestApplyOneNoopSkipsApply(t *testing.T) {
	...
	outs, _, err := d.applyOne(context.Background(), g, node, map[string]interface{}{}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if c.applyCalls != 0 {
...
func TestApplyOneReplaceRefusedByPreventDestroy(t *testing.T) {
	...
	_, _, err := d.applyOne(context.Background(), g, node, map[string]interface{}{}, nil)
	if err == nil {
