...
- [x] 1.1 `nix/example/tutorial-features-0.4/`: move the current `tutorial.nix`
...
- [x] 1.2 `nix/example/tutorial-getting-started/`: a from-scratch starter built on
      the headline two-provider round trip (from `default.nix`): `flake.nix`,
      config, `README.md`.
- [x] 1.3 Repo flake: point `nivis.tutorial` at the features-0.4 starter's config
...
- [x] 1.4 `tests/run-nix-tests.sh` / conformance: each starter config still
      produces conforming IR; the bare-name `source` is unchanged.

## 2. nivistutor binary
- [x] 2.1 `cmd/nivistutor`: embed the starter directories with `go:embed`
...
- [x] 2.2 A non-interactive mode (flags: `--tutorial <name> --dir <path>`) for
      scripting/tests; refuse to overwrite existing files without `--force`.
- [x] 2.3 cobra command + a branded greeting consistent with `nivis`' splash.

## 3. Flake wiring
- [x] 3.1 `flake.nix`: a `nivistutor` package (buildGoModule, `cmd/nivistutor`)
...
## 4. Tests
- [x] 4.1 `cmd/nivistutor`: non-interactive scaffold writes the expected files
      (flake.nix + config + README) for each tutorial into a temp dir.
- [x] 4.2 No-clobber: scaffolding over an existing file without `--force` errors,
      state unchanged.
- [x] 4.3 Each embedded starter has a `flake.nix` (guard against a tutorial added
      without its flake).

## 5. Docs
- [x] 5.1 The feature tutorial + getting-started: a short "scaffold it with
      nivistutor" path for a sandbox (`nix shell …#nivis …#tutor; nivistutor`).
      No em dashes.
- [x] 5.2 If the feature-tutorial doc moves/renames for the per-release scheme,
...
- [x] 6.3 `openspec validate nivistutor --strict`; archive; update beans-97jm
