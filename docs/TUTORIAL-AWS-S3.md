...
- The **object** is the interesting part. `bucket = bucket.refAttr "id"` is a
  *reference* to the bucket's output, so terrae nivis knows the object depends on
  the bucket. And `content = tn.str [ … (bucket.refAttr "id") … ]` builds the
...
because the object's content needs the bucket's name. terrae nivis then
re-evaluates the Nix config with the bucket's generated name injected, which
resolves the object's `content`; phase 2 uploads it. `tn` writes the resulting
state to `terrae-nivis.state.json` in `my-infra`.
...
```sh
tn state show aws.aws_s3_object.note
```
...
```sh
aws s3 cp "s3://$(tn state show aws.aws_s3_bucket.demo | awk '/^  id = /{print $3}')/hello-from-nix.txt" -
```
